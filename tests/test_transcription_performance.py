...
class TranscriptionPerformanceTests(unittest.TestCase):
    ...
    def test_metrics_reject_content_unknown_fields_and_nonfinite_numbers(self):
        self.assertEqual(
            safe_timings(
                {
                    "text": "private",
                    "provider_ms": 12.34567,
                    "delivery_ms": float("inf"),
                    "asr_ms": True,
                    "inference_ms": -1,
                }
            ),
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(local_asr, "memory_bytes", return_value=None):
                ...
                try:
                    ...
                    while (
                        manager.process_id is not None and time.monotonic() < deadline
                    ):
                        time.sleep(0.01)
                    manager.shutdown()
    ...
    def test_prepare_skips_low_memory_and_already_finished_recordings(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(
                local_asr, "memory_bytes", return_value=(4 * 1024**3, 512 * 1024**2)
            ):
    ...
    def test_local_request_reports_start_and_inference_without_audio_content(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                manager.transcribe(
                    Path("test.wav"), "pt", audio_bytes=b"RIFFaudio", timings=timings
                )
    ...
    def test_delivery_precedes_disk_statistics_and_timings_exclude_persistence(self):
        ...
        service = WorkflowService(
            ...
            audio,
            clipboard,
            workflow_fixtures.FakeConfig(),
            stats,
            workflow_fixtures.ImmediateScheduler(),
            clock,
        )
    ...
    def test_recording_reads_only_after_recorder_finalization_without_sleep(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(
                qml_runtime, "_data_directory", return_value=Path(directory)
            ):
                session = qml_runtime.QtRecordingSession(recorder)
                recorder.stop.side_effect = lambda: session.audio_path.write_bytes(
                    b"RIFF" + b"0" * 1200
                )
                session.start()
                with patch.object(
                    qml_runtime.time, "sleep", side_effect=AssertionError("fixed sleep")
                ):
    ...
    def test_idle_watch_rechecks_memory_pressure(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (
                patch.object(local_asr.threading, "Timer", side_effect=timer),
