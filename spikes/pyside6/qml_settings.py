...
class QmlSettingsController(QObject):
    ...
    def refreshRouteModels(self) -> bool:
        ...
        threading.Thread(
            target=discover, name="ClarifyVoiceModelDiscovery", daemon=True
        ).start()
        return True
    ...
    def _browse_local(self, model, device):
        try:
            ...
            product = LocalASRProductController(installer=ModelInstaller(model, adapter.engines, self._local_devices), backend=backend)
        except Exception as error:
            self._set_error(error)
            return False
