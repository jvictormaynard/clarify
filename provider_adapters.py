...
class ProviderAdapter:
    ...
    def discover_models(
        self, connection: ProviderConnection, cancel_token=None
    ) -> ModelCatalog:
        payload = self.validate(connection, cancel_token, operation="model_discovery")
        return ModelCatalog(
            self.parse_audio_models(payload),
            self.parse_text_models(payload),
        )

    def fetch_audio_models(
        self, connection: ProviderConnection, cancel_token=None
    ) -> tuple[str, ...]:
    ...
    def rewrite(
        self, request: RewriteRequest, connection: ProviderConnection, cancel_token=None
    ) -> RewriteResult:
...
class GeminiAdapter(ProviderAdapter):
    ...
    def parse_text_models(self, payload: Mapping[str, Any]) -> tuple[str, ...]:
        return tuple(
            model
            for model in self._parse_models(payload)
            if not any(token in model.lower() for token in self._TEXT_MODEL_EXCLUSIONS)
        )
    ...
    def _generate(
        ...
        response = self.http.request(
            ...
            normalize_provider_url(
                connection.base_url, self._VERSION, f"models/{model_id}:generateContent"
            ),
            provider=self.metadata.provider_id,
            operation=operation,
            cancel_token=cancel_token,
...
class OpenAICompatibleAdapter(ProviderAdapter):
    ...
    def parse_audio_models(self, payload: Mapping[str, Any]) -> tuple[str, ...]:
        models = {
            ...
            for model_id in (self._model_id(entry),)
            if model_id
            and any(token in model_id.lower() for token in ("whisper", "transcribe"))
        }
        return tuple(sorted(models, key=str.lower))
        ...
        try:
            transcription_data = {
                ...
                "prompt": request.effective_prompt(
                    max_chars=896 if self.metadata.provider_id == "groq" else None
                ),
            }
            ...
            audio_file.close()
        payload = self.http.json(
            response, provider=self.metadata.provider_id, operation="transcription"
        )
        text = (
            str(payload.get("text", "")).strip() if isinstance(payload, Mapping) else ""
        )
        if not text:
            raise _invalid_response_error(
